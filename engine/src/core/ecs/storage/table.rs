...
/// let mut table = Table::new(spec, &mut component_registry);
///
/// // Create an entity
...
pub struct Table {
    ...
    /// Map from component ID to column index for fast lookup.
    component_to_column: HashMap<component::Id, usize>,
}
...
impl Table {
    ...
    ///
    pub fn new(components: component::Spec, registry: &mut component::Registry) -> Self {
        let mut component_to_column = HashMap::new();
        let mut columns = Vec::new();

        for (index, &component_id) in components.ids().iter().enumerate() {
            component_to_column.insert(component_id, index);
            let info = registry
                .get_info_by_id(component_id)
    ...
    pub fn get_column(&self, component_id: component::Id) -> Option<&Column> {
        self.component_to_column
            .get(&component_id)
            .and_then(|&idx| self.columns.get(idx))
    }

    /// Get a mutable reference to a column by component ID.
    pub fn get_column_mut(&mut self, component_id: component::Id) -> Option<&mut Column> {
        self.component_to_column
            .get(&component_id)
    ...
    pub unsafe fn get<C: Component>(&self, row: Row, component_id: component::Id) -> Option<&C> {
        let column = self.get_column(component_id)?;
    ...
    pub unsafe fn get_mut<C: Component>(
        ...
        let column = self.get_column_mut(component_id)?;
        unsafe { column.get_mut(row) }
    }
...
impl SetTarget for Table {
    ...
    /// - The type `C` matches the column's component type.
...
mod tests {
    ...
    fn table_creation_with_default_index() {
        ...
        // When
        let table = Table::new(spec, &mut component_registry);

        // Then
        assert_eq!(table.len(), 0);
        assert_eq!(table.columns.len(), 2);
        // Columns are None until first component is added
        assert!(table.get_column(pos_id).is_some());
        assert!(table.get_column(vel_id).is_some());
    }
    ...
    fn table_entity_management() {
        ...
        // When
        let row1 = table.add_entity(entity1, Health { value: 100 }, &mut component_registry);
        let row2 = table.add_entity(entity2, Health { value: 75 }, &mut component_registry);
        ...
        let column = table.get_column(health_id).unwrap();
        unsafe {
    ...
    fn table_with_multiple_component_types() {
        ...
        let vel_column = table.get_column(vel_id).unwrap();
        let health_column = table.get_column(health_id).unwrap();
        unsafe {
    ...
    fn table_column_iteration() {
        ...
        let column = table.get_column(score_id).unwrap();
        unsafe {
        ...
        // Mutate all scores
        let column = table.get_column_mut(score_id).unwrap();
        unsafe {
    ...
    fn test_table_incomplete_row_panics() {
        ...
        table.add_entity(entity, Position {}, &mut component_registry);
    }
    ...
    fn table_swap_remove_row() {
        ...
        table.add_entity(entity1, Value { n: 100 }, &mut component_registry);
        table.add_entity(entity2, Value { n: 200 }, &mut component_registry);
        table.add_entity(entity3, Value { n: 300 }, &mut component_registry);
        ...
        // Verify column data
        let column = table.get_column(value_id).unwrap();
        unsafe {
    ...
    fn table_swap_remove_row_out_of_bounds() {
        ...
        let mut allocator = entity::Allocator::new();
        table.add_entity(allocator.alloc(), Comp {}, &mut component_registry);
    ...
    fn table_get_mut_component_by_row() {
        ...
        let row = table.add_entity(entity, Health { value: 100 }, &mut component_registry);

        // When - mutate the health
        unsafe {
            let health = table.get_mut::<Health>(row, health_id);
        ...
        unsafe {
            let health = table.get::<Health>(row, health_id);
    ...
    fn table_components_accessor() {
        ...
        let table = Table::new(spec.clone(), &mut component_registry);
    ...
    fn table_get_column_none_for_invalid_id() {
        ...
        // Then
        assert!(table.get_column(comp2_id).is_none());
        assert!(table.get_column_mut(comp2_id).is_none());
    }
    ...
    fn table_get_returns_none_for_entity_not_in_table() {
        ...
        table.add_entity(entity, Value { n: 100 }, &mut component_registry);

        // When/Then - entity not in table returns None
        unsafe {
            assert_eq!(table.get::<Value>(1.into(), value_id), None);
        }
    }
}
