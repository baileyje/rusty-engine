...
impl<'w> Ref<'w> {
    ...
    #[inline]
    pub fn components(&self) -> &'w component::Spec {
...
mod tests {
    ...
    fn ref_get_existing_component() {
        ...
        let row = table.add_entity(entity, (position.clone(), velocity.clone()), &mut registry);
    ...
    fn ref_get_nonexistent_component() {
        ...
        let position = Position { x: 10.0, y: 20.0 };
        let row = table.add_entity(entity, (position,), &mut registry);
    ...
    fn ref_for_nonexistent_entity() {
        ...
        let position = Position { x: 10.0, y: 20.0 };
        table.add_entity(entity1, (position,), &mut registry);
